...
現在 checkout 中の作業ブランチ（ユーザーが事前にリモートへ push 済みである前提）を使って、GitHub MCP 経由で Pull Request を作成する。
（AIはコミット/プッシュを行わない。必要な場合は事前に許可を取る）
Backlog の情報が必要な場合は、Backlog MCP を適宜利用してください。


# 最優先: 運用ポリシー（再発防止）

- デフォルトでコミットしない。デフォルトでプッシュしない。リモートブランチを直接更新しない。
...
- PR作成（GitHub MCPの `create_pull_request` 等）はGitHub側への書き込みを伴うため、実行前に「PRを作成してよいですか？」と確認し、承諾後にのみ実行する。
